...
object Point:
  def atInfinity[C: Coordinate, A <: C: ValueOf, B <: C: ValueOf]
      : Point[C, A, B] =
    PointAtInfinity()

  def apply[C: Coordinate, A <: C: ValueOf, B <: C: ValueOf](
      x: C,
      y: C
  ): Point[C, A, B] =
    NonZeroPoint(x, y)

  given pointRMul[C: Coordinate, A <: C: ValueOf, B <: C: ValueOf]
      : RMul[Point[C, A, B]] with
    def rmul(coeff: BigInt, p: Point[C, A, B]) = p.rmul(coeff)

final case class PointAtInfinity[
    C: Coordinate,
    A <: C: ValueOf,
    B <: C: ValueOf
]() extends Point[C, A, B]:
...
final case class NonZeroPoint[C: Coordinate, A <: C: ValueOf, B <: C: ValueOf](
    x: C,
    y: C
) extends Point[C, A, B]:
  ...
  override def rmul(s: BigInt) = LazyList
    .unfold((s, this: Point[C, A, B])): (i, c) =>
      Option.when(i > 0) { ((i & 1, c), (i >> 1, c + c)) }
    .collect:
      case (1, c) => c

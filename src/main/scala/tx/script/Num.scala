...
object Num:
  ...
  def encode(num: BigInt): Elem =
    if num == 0 then Array[Byte]()
    else
      val result = LazyList
        .unfold(num.abs)(absNum =>
          Option.when(absNum != 0)(((absNum & 0xff).toByte, absNum >> 8))
        )
        .toArray

      if (result.last & 0x80) != 0 then
        if num < 0 then result :+ 0x80.toByte
        else result :+ (0: Byte)
      else if num < 0 then result.init :+ (result.last | 0x80).toByte
      else result

  def decode(element: Elem): BigInt =
    if element.isEmpty then 0
    else
